...
**Q: Дубликаты картинок (файлы вида 2000019668213_1.jpg, 2000019668213_1-1.jpg, 2000019668213_1-2.jpg).**
A: Начиная с **v1.9.5** проблема решена кардинально:
1. **MySQL GET_LOCK** на каждый basename — исключает race condition между AJAX и WP-Cron
2. **Прямой `wp_insert_attachment()`** вместо `media_handle_sideload()` — никакого авто-добавления `-1`, `-2` суффиксов
3. **Детерминированное имя файла** — всегда `basename.ext`, при совпадении файл перезаписывается
4. **Корректное экранирование `_` в SQL LIKE** через `ESCAPE '\\'` — раньше `_` работал как wildcard и находил ложные совпадения
5. **Двойная проверка** после получения лока (защита от окна между проверкой и созданием)

Если дубликаты уже накопились — нажмите **«Удалить только дубликаты картинок»** (оранжевая кнопка на странице импорта).
