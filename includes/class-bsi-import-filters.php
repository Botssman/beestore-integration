...
class BSI_Import_Filters {
	...
	/**
	 * Получить список всех уникальных категорий и брендов из CSV-файла.
	 * Используется в админке для отображения списка доступных категорий/брендов.
	 *
	 * @param string $csv_file Путь к CSV.
	 * @return array [ 'categories' => [...], 'brands' => [...] ]
	 */
	public function scan_csv_for_filters( $csv_file ) {
		...
		foreach ( $parser as $row ) {
			...
			if ( ! empty( $row['DSCategoriaMerceologicaWeb'] ) ) {
				$cat = $row['DSCategoriaMerceologicaWeb'];
			} elseif ( ! empty( $row['DSCategoriaMerceologica'] ) ) {
				$cat = $row['DSCategoriaMerceologica'];
			}
