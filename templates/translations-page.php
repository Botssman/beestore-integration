        ...
        <div class="bsi-card">
                ...
                <?php else : ?>
                        ...
                        <p style="margin-top:15px;">
                                ...
                                <button type="button" class="button button-secondary" id="bsi-merge-duplicates" style="margin-left:10px;border-color:#d63638;color:#d63638;">
                                        <span class="dashicons dashicons-admin-page"></span>
                                        <?php esc_html_e( 'Слить дубли (slug -2, -3)', 'beestore-integration' ); ?>
                                </button>
...
jQuery(document).ready(function($){
        ...
        // ─── Слияние дублей категорий ──────────────────────────────
        $('#bsi-merge-duplicates').on('click', function() {
                if (!confirm('Слить дубли категорий? Товары из дублей (slug -2, -3) будут перенесены в оригиналы. Дубли будут удалены. Продолжить?')) return;
                var $btn = $(this);
                var $status = $('#bsi-translations-status');
                $btn.prop('disabled', true);
                $status.html('<span class="spinner is-active" style="float:none;vertical-align:middle;"></span> Сливаем...');

                $.post(bsiAdmin.ajaxUrl, {
                        action: 'bsi_merge_duplicate_terms',
                        nonce: bsiAdmin.nonce,
                        taxonomy: currentTax
                }, function(response) {
                        $btn.prop('disabled', false);
